...
const AvailableAppointment = ({ selectedDate }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://hello-doctors-server.vercel.app/appointmentOptions?date=${date}`
      );
      const data = res.json();
      return data;
    },
  ...
  if (isLoading) {
    return (
      <span className="loading loading-ring loading-lg text-green-800 mx-[450px]"></span>
    );
  }
  ...
  return (
    <div>
      <p className="text-center text-green-500 text-lg font-semibold">
        ...
        <span className="text-white bg-gray-500 px-5 rounded-md">
